...
namespace Microsoft.Tests
{
    public class DashTestBase
    {
        ...
        protected class DashTestContext
        {
            ...
            public IMessageQueue WorkerQueue { get; set; }
        ...
        protected static DashTestContext InitializeConfigAndCreateTestBlobs(TestContext testCtx, 
            ...
            Func<DashTestContext> contextCreateFactory = null)
        {
        ...
        protected static DashTestContext InitializeConfig(TestContext testCtx, 
            string configurationName, 
            IDictionary<string, string> config, 
            string containerPrefix = "", 
            Func<DashTestContext> contextCreateFactory = null)
        {
            ...
            while (true)
            {
                if (!config.ContainsKey(DashConfiguration.KeyScaleoutAccountPrefix + nextDataAccountIndex.ToString()))
                {
                    break;
                }
            ...
            var augmentedConfig = config
                .Concat(secretsConfig.DataConnectionStrings
                    .Select((connectString, index) => new KeyValuePair<string, string>(DashConfiguration.KeyScaleoutAccountPrefix + (nextDataAccountIndex + index).ToString(), connectString)))
                .ToDictionary(item => item.Key, item => item.Value, StringComparer.OrdinalIgnoreCase);
            if (!String.IsNullOrWhiteSpace(secretsConfig.NamespaceConnectionString))
            {
                augmentedConfig[DashConfiguration.KeyNamespaceAccount] = secretsConfig.NamespaceConnectionString;
            }
            if (contextCreateFactory == null)
            {
                contextCreateFactory = () => new DashTestContext();
            }
            var retval = contextCreateFactory();
            retval.Runner = new WebApiTestRunner(augmentedConfig);
            retval.ContainerName = containerPrefix + Guid.NewGuid().ToString("N");
            if (augmentedConfig.ContainsKey(DashConfiguration.KeyWorkerQueueName))
            {
                retval.WorkerQueue = new AzureMessageQueue();
            }
            return retval;
        }
